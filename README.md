# autounifi
autounifi is an Python Script to automate Access Point Radio Configuration in a Ubiquiti Unifi Controller

You can configure AP radio configurations in seconds every one at the same time
We only need an Excel file where parameters are especified (for example: Channel, Channel Width, Power Mode, RSSI, etc.)

The Python Script get these parameters from the Excel file and through UniFi REST API sends them to UniFi Controller to apply changes.
